...
export async function POST(req: NextRequest) {
  try {
    ...
    // Optional: Validate frame message
    if (body.untrustedData?.messageBytes) {
      const validation = await neynarClient.validateFrameAction({
        message_bytes_in_hex: body.untrustedData.messageBytes,
      });
      
      if (validation.valid) {
        console.log(`Valid frame action from FID: ${validation.action?.interactor.fid}`);
      }
    }
